...
class CurrencyControllerTest extends BaseMvcTest {

    @Test
    void should_not_get_without_authentication() throws Exception {
        mvc.perform(get(PATA_PATH +  CURRENCY_PATH + GET_METHOD)).andExpect(jsonPath(ROOT + code).value(TOKEN_MISSING));
    }

    @Test
    @Sql(statements = INSERT_CURRENCY + "(5,'USD','USD',1,1,1,100000,'$')," +
            "(258,'VND','Vietnamese Dong',1,1,2000,2310000000,'₫')")
    void should_get_all_correctly() throws Exception {
        mvc.perform(get(PATA_PATH +  CURRENCY_PATH + GET_METHOD).header(BOOST_CORE_SECURITY_TOKEN_NAME.value(), TOKEN_NORMAL_USER)).andExpect(jsonPath(ROOT + data + '.' + results, hasSize(2)));
    }
    ...
    void should_get_page_correctly() throws Exception {
        mvc
                .perform(get(PATA_PATH +  CURRENCY_PATH + GET_METHOD).header(BOOST_CORE_SECURITY_TOKEN_NAME.value(), TOKEN_NORMAL_USER).param(size, "1"))
