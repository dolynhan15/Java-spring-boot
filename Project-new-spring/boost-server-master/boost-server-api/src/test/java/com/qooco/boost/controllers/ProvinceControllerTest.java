...
class ProvinceControllerTest extends BaseMvcTest {

    @Test
    void should_not_get_without_authentication() throws Exception {
        mvc.perform(get(PATA_PATH + PROVINCE_PATH + GET_METHOD)).andExpect(jsonPath(ROOT + code).value(TOKEN_MISSING));
    }
    ...
    void should_get_all_correctly() throws Exception {
        mvc
                .perform(get(PATA_PATH + PROVINCE_PATH + GET_METHOD).header(BOOST_CORE_SECURITY_TOKEN_NAME.value(), TOKEN_NORMAL_USER).param(countryId, "93"))
    ...
    void should_get_none_if_wrong_id() throws Exception {
        mvc
                .perform(get(PATA_PATH + PROVINCE_PATH + GET_METHOD).header(BOOST_CORE_SECURITY_TOKEN_NAME.value(), TOKEN_NORMAL_USER).param(countryId, "0"))
    ...
    void should_get_page_correctly() throws Exception {
        mvc
                .perform(get(PATA_PATH + PROVINCE_PATH + GET_METHOD).header(BOOST_CORE_SECURITY_TOKEN_NAME.value(), TOKEN_NORMAL_USER).param(countryId, "93").param(size, "1"))
